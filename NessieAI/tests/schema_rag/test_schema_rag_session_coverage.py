...
class TestLoadSessionInfo:
    ...
    @patch("NessieAI.schema_rag.session.duckdb")
    @patch("NessieAI.schema_rag.session.get_db_path_for_session", return_value="/tmp/test.duckdb")
    @patch("NessieAI.schema_rag.session.os.path.exists", return_value=True)
    def test_returns_none_when_no_result(self, mock_exists, mock_path, mock_duckdb):
        from NessieAI.schema_rag.session import load_session_info
        mock_conn = MagicMock()
...
class TestDeleteSession:
    ...
    @patch("NessieAI.schema_rag.session.get_db_path_for_session", return_value="/tmp/nonexistent.duckdb")
    @patch("NessieAI.schema_rag.session.os.remove", side_effect=FileNotFoundError)
    def test_file_not_found_silently_ignored(self, mock_remove, mock_path):
        from NessieAI.schema_rag.session import delete_session
        # Should not raise
        delete_session("nonexistent-session")

    @patch("NessieAI.schema_rag.session.get_db_path_for_session", return_value="/tmp/existing.duckdb")
    @patch("NessieAI.schema_rag.session.os.remove")
...
class TestCleanupExpiredSessions:
    ...
    @patch("NessieAI.schema_rag.session.os.path.isdir", return_value=False)
    @patch("NessieAI.schema_rag.session.settings")
    def test_no_directory(self, mock_settings, mock_isdir):
        from NessieAI.schema_rag.session import cleanup_expired_sessions
        mock_settings.SCHEMA_RAG_DUCKDB_DIR = "/nonexistent"
        # Should not raise
        cleanup_expired_sessions()

    @patch("NessieAI.schema_rag.session.delete_session")
    @patch("NessieAI.schema_rag.session.load_session_info", return_value=None)
    @patch("NessieAI.schema_rag.session.os.listdir", return_value=["orphan123.duckdb"])
    @patch("NessieAI.schema_rag.session.os.path.isdir", return_value=True)
    @patch("NessieAI.schema_rag.session.settings")
    ...
    @patch("NessieAI.schema_rag.session.delete_session")
    @patch("NessieAI.schema_rag.session.is_session_expired", return_value=True)
    @patch("NessieAI.schema_rag.session.load_session_info")
    @patch("NessieAI.schema_rag.session.os.listdir", return_value=["expired.duckdb"])
    @patch("NessieAI.schema_rag.session.os.path.isdir", return_value=True)
    @patch("NessieAI.schema_rag.session.settings")
    ...
    @patch("NessieAI.schema_rag.session.load_session_info", side_effect=Exception("corrupt"))
    @patch("NessieAI.schema_rag.session.os.listdir", return_value=["corrupt.duckdb"])
    @patch("NessieAI.schema_rag.session.os.path.isdir", return_value=True)
    @patch("NessieAI.schema_rag.session.settings")
    ...
    @patch("NessieAI.schema_rag.session.os.listdir", return_value=["readme.txt", "test.csv"])
    @patch("NessieAI.schema_rag.session.os.path.isdir", return_value=True)
    @patch("NessieAI.schema_rag.session.settings")
