...
class TestApplyResolvedTermsBoost:
    ...
    def test_no_resolved_terms(self):
        from NessieAI.schema_rag.service import _apply_resolved_terms_boost
...
class TestComputeScores:
    ...
    def test_empty_texts(self, mock_embed):
        from NessieAI.schema_rag.service import _compute_scores
...
class TestBuildEnrichedText:
    ...
    def test_with_examples(self):
        from NessieAI.schema_rag.service import _build_enriched_text
...
class TestFlattenEndpoints:
    ...
    def test_non_dict_path_item_skipped(self):
        from NessieAI.schema_rag.service import flatten_endpoints
        processor = MagicMock()
    ...
    def test_missing_operation_id_synthesized(self):
        ...
        with patch("NessieAI.schema_rag.service.settings") as mock_settings:
...
class TestFindSessionBySchemaUrl:
    ...
    def test_no_duckdb_dir(self, mock_settings, mock_isdir):
        from NessieAI.schema_rag.service import _find_session_by_schema_url
    ...
    @patch("NessieAI.schema_rag.service.load_session_info", side_effect=Exception("corrupt"))
    @patch("os.listdir", return_value=["abc123.duckdb"])
    @patch("os.path.isdir", return_value=True)
    @patch("NessieAI.schema_rag.service.settings")
