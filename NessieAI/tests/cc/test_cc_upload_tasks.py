...
def _orig():
    fn = tasks.run_cc_upload_task
    while hasattr(fn, "__wrapped__"):
        fn = fn.__wrapped__
    return getattr(fn, "__func__", fn)


def test_run_cc_upload_moves_files_and_unlinks_tmp(tmp_path: Path):
    dest = tmp_path / "input"
    a = tmp_path / "a.csv"
    b = tmp_path / "b.txt"
    a.write_text("A")
    ...
    assert self.states[-1]["state"] == "PROGRESS"
    assert self.states[-1]["meta"]["progress_pct"] == 100


def test_run_cc_upload_empty_files_still_succeeds(tmp_path: Path):
    dest = tmp_path / "input"
    result = _orig()(_Self(), input_mnt=str(dest), files=[])
...
def test_run_cc_upload_rejects_traversal_name_and_cleans_tmp(tmp_path: Path):
    ...
    with pytest.raises(Exception):
        _orig()(_Self(), input_mnt=str(dest), files=[{"name": "../x", "tmp_path": str(staged)}])
