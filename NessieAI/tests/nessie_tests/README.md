...
| `NessieAI/tests/nessie_tests/evaluate.py:665` | `evaluate_turn`, criterion scoring for one turn |
...
uv run --no-project --with pytest --with pydantic --with requests --with beautifulsoup4 \
  --with orjson python -m pytest NessieAI/tests/nessie_tests/tests -q -p no:cacheprovider
```
...
```bash
uv run --no-project --with pytest --with pydantic --with requests --with beautifulsoup4 \
  --with orjson python -m pytest NessieAI/tests/nessie_tests/tests -q -p no:cacheprovider 2>&1 | tail -1
```
...
```bash
python -m NessieAI.tests.nessie_tests --base-url http://localhost:8000 --tier route --scope specific
```
...
- Two mutation-testing contracts name `NessieAI/tests/nessie_tests/v4_2_verifier.py` as the file under mutation (`scripts/plan018_v4_9_task5_mutation.py:105-106`).

**Depends on:**
- The vendored `chat_nextseek` e2e criterion DSL, reached by a `sys.path` insertion performed at import time (`nessie_tests/pathsetup.py:10-16`), which two modules trigger at module scope (`NessieAI/tests/nessie_tests/corpus.py:6-9`, `NessieAI/tests/nessie_tests/evaluate.py:10`).
...
- Three heavy dependencies are deliberately lazy, imported inside the function that needs them: Django models (`NessieAI/tests/nessie_tests/bundle.py:30`), `openpyxl` (`NessieAI/tests/nessie_tests/evaluate.py:151`) and `zstandard` (`NessieAI/tests/nessie_tests/collect.py:177`).
...
`nextseek_api.assistant.models_db` at `NessieAI/tests/nessie_tests/sources.py:254` sits inside
