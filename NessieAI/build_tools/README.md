...
`NessieAI/tests/build_tools/` (all counted by a `find` over `build_tools`). It carries
...
the Container-CC operation registry (`NessieAI/build_tools/gen_op_surfaces/emit.py:217-235`),
and `ingest_nextseek_docs` refreshes the vendored NExtSEEK user-docs snapshot from
GitBook (`NessieAI/build_tools/ingest_nextseek_docs/constants.py:9-12`). The five `plan005_*`
...
(`NessieAI/history/plan005/plan005_closeout.py:21`) and to two image digests
(`NessieAI/history/plan005/plan005_closeout.py:15-20`).

`ingest_nextseek_docs` came from the dmac-assistant repository. The cc-runtime port
record pins source commit `a429f137`
(`docker/cc-runtime/PORT-EVIDENCE.json:2-4`) and names the
...
(`NessieAI/build_tools/ingest_nextseek_docs/constants.py:14-15`) and two helpers for GitBook's
...
rendering (`NessieAI/build_tools/gen_op_surfaces/__main__.py:14-41`). `--check` renders every
...
(`NessieAI/build_tools/gen_op_surfaces/constants.py:4-6`).

The target registry (`NessieAI/build_tools/gen_op_surfaces/emit.py:217-235`) resolved to 12
targets on this tree on 2026-09-03: 2 whole-file and 10 marked blocks over 8 files.

| Generated target | Kind | Emitter |
|---|---|---|
| `dmac_assistant/build_context/route_capabilities.json` | whole file | `NessieAI/build_tools/gen_op_surfaces/route_capabilities.py:326-327` |
| `docker/cc-runtime/build_context/plugins/nextseek/context/capabilities.md` | whole file | `NessieAI/build_tools/gen_op_surfaces/emit.py:81-88` |
| `docker/cc-runtime/Dockerfile` plugin `COPY`, plugin `PATH`, capabilities `COPY` | 3 blocks | `NessieAI/build_tools/gen_op_surfaces/docker_blocks.py:37-62` |
| `docker-compose.yml` additional build contexts | 1 block | `NessieAI/build_tools/gen_op_surfaces/docker_blocks.py:65-70` |
| `docker/cc-runtime/container/CLAUDE.md` plugin, skill and operation inventories | 3 blocks | `NessieAI/build_tools/gen_op_surfaces/claude_md.py:144-188` |
| each plugin `commands/*.md` carrying the command-ops markers | 1 block each | `NessieAI/build_tools/gen_op_surfaces/commands.py:34-64` |
| each installed `SKILL.md` | 1 block each | `NessieAI/build_tools/gen_op_surfaces/skills.py:105-144` |
...
(`NessieAI/build_tools/gen_op_surfaces/commands.py:67-88`), skills from the install oracle's
own discovery (`NessieAI/build_tools/gen_op_surfaces/skills.py:147-167`), and the Dockerfile,
...
(`NessieAI/build_tools/gen_op_surfaces/constants.py:8-13`). The emitter for the baked copy is
...
(`NessieAI/build_tools/gen_op_surfaces/docker_blocks.py:56-62`) and validates as the final
writer of that in-image path
(`NessieAI/build_tools/gen_op_surfaces/docker_blocks.py:133-159`).

Because targets are rendered in sorted order by path
(`NessieAI/build_tools/gen_op_surfaces/emit.py:235`), `dmac_assistant/...` sorts ahead of
`docker/...`, so `route_capabilities.json` is rendered first and its own
byte-identity precondition on the capabilities pair
(`NessieAI/build_tools/gen_op_surfaces/route_capabilities.py:229-232`) is what a stale baked
...
(`NessieAI/build_tools/plan005_validate_plugins/__main__.py:19-50`). The validator image is
pinned to one digest and any substitute is refused
(`NessieAI/build_tools/plan005_validate_plugins/validate.py:73-78`).

### The Plan 005 evidence protocol

Five modules that together record, gate and close out one historical plan.
`NessieAI/history/plan005/plan005_closeout.py:40-57` fixes the 16 ordered record IDs;
`NessieAI/history/plan005/plan005_closeout.py:643-654` exposes the `protocol`, `preflight`,
`finalize` and `verify` stages, whose control logic lives in
`NessieAI/history/plan005/plan005_closeout_control.py:1`. `NessieAI/history/plan005/plan005_record.py:1`
records one command's evidence without a shell, refusing secret-bearing argv
(`NessieAI/history/plan005/plan005_record.py:79`). `NessieAI/history/plan005/plan005_baseline.py:1`
materializes the exact base-commit blobs outside the repo.
`NessieAI/history/plan005/plan005_gate.py:1` is the coverage / JUnit / pragma gate; it maps
changed test files onto five named CI lanes
(`NessieAI/history/plan005/plan005_gate.py:210-220`) and enforces a floor of 95 percent combined
branch-enabled coverage (`NessieAI/history/plan005/plan005_closeout.py:24`, applied at
`NessieAI/history/plan005/plan005_gate.py:362-369`). `NessieAI/history/plan005/schemas/plan005-closeout.schema.json:9-13`
...
(`NessieAI/history/plan005/plan005_signoffs/task-09-compose-dockerfile.json:7-11`).

## Running and testing

One lane, and it is genuinely runnable. `NessieAI/tests/build_tools/` holds 20 `test_*.py`
modules under `NessieAI/tests/build_tools/unit/`, `NessieAI/tests/build_tools/integration/` and the
...
(`NessieAI/tests/build_tools/integration/test_markdown_source_contract.py:44`), as does every
other test that touches GitBook (`NessieAI/tests/build_tools/unit/test_fetch.py:36`).
...
(`NessieAI/tests/build_tools/unit/test_gen_op_surfaces.py:272`, asserting at
`NessieAI/tests/build_tools/unit/test_gen_op_surfaces.py:308-315`):

```sh
docker run --rm -v "$PWD":/src:ro -w /src -e PYTHONDONTWRITEBYTECODE=1 \
  --entrypoint sh nextseek-nextseek:latest \
  -c '/app/.venv/bin/python -m pytest NessieAI/tests/build_tools -q -p no:cacheprovider'
```
...
(`NessieAI/tests/build_tools/unit/test_gen_op_surfaces_claude_md.py:66-67`) which the
...
(`NessieAI/history/plan005/plan005_closeout_control.py:496-500`).
...
(`NessieAI/history/plan005/plan005_closeout.py:15-20`) that `docker image inspect` reports as
absent from this host (checked 2026-09-03), plus an evidence tree under a path that
does not exist here (`NessieAI/history/plan005/plan005_closeout.py:25`).
...
- `nextseek_api.cc_assistant.op_registry` is the data source for every generated op
  ...
  `NessieAI/build_tools/gen_op_surfaces/skills.py:11-13`,
  `NessieAI/build_tools/gen_op_surfaces/claude_md.py:16-21`,
  `NessieAI/build_tools/gen_op_surfaces/docker_blocks.py:12-17` and
  `NessieAI/build_tools/gen_op_surfaces/route_capabilities.py:8-26`.
- `NessieAI/build_tools/plan005_validate_plugins/validate.py:9-17` is the sixth importer of
...
- `nessie_tests` is imported at module scope by the route-capabilities generator for
  ...
  (`NessieAI/build_tools/gen_op_surfaces/route_capabilities.py:27-29`), and the corpus file
  itself is read by relative path (`NessieAI/build_tools/gen_op_surfaces/route_capabilities.py:39`).
- `dmac_assistant.router.capabilities` is imported lazily inside a function, so the
  ...
  returning it (`NessieAI/build_tools/gen_op_surfaces/route_capabilities.py:306-323`).
...
- Django is **not** a dependency, even though every generator reads a Django app's
  ...
  (`NessieAI/build_tools/gen_op_surfaces/route_capabilities.py:4-29`, counted 2026-09-03); no
...
- External binaries. `git` is invoked as a subprocess by the gate
  (`NessieAI/history/plan005/plan005_gate.py:147-155`), the baseline materializer
  (`NessieAI/history/plan005/plan005_baseline.py:31-41`) and the closeout control
  (`NessieAI/history/plan005/plan005_closeout_control.py:59-68`); `docker` by the plugin validator
...
- Load-bearing input directories the tools READ, not scratch they write:
  ...
  anywhere under `build_tools` (`NessieAI/build_tools/gen_op_surfaces/commands.py:11`,
  `NessieAI/build_tools/gen_op_surfaces/skills.py:15`,
  `NessieAI/build_tools/gen_op_surfaces/claude_md.py:23`,
  `NessieAI/build_tools/gen_op_surfaces/docker_blocks.py:19`,
  `NessieAI/build_tools/gen_op_surfaces/route_capabilities.py:40`,
  `NessieAI/build_tools/plan005_validate_plugins/validate.py:19`).
  `build_tools/plan005_signoffs/` is globbed for approval records
  (`NessieAI/history/plan005/plan005_closeout_control.py:485`). Move either and the affected
...
- By path string, not by import: `NessieAI/history/plan005/plan005_gate.py:26-28` pins three test
...
- Deployment runbook: `DEPLOYMENT.md:612-613` names
  `python -m NessieAI.build_tools.ingest_nextseek_docs` as the only sanctioned way to refresh
...
- Generated-file provenance: `docker/cc-runtime/docs/nextseek/README.md:3` carries a
  do-not-edit banner naming `NessieAI/build_tools/ingest_nextseek_docs`.
