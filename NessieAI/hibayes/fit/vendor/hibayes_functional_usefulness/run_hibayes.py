"""T3.2 — Stage D in-image entry point for the functional-usefulness axis.
...
# defeat the smoke gate (see plan DL-028 + this spec's D5 fix-log).
...
from .load_csv import load_functional_usefulness_csv
from .models import PosteriorTaskFamilyReliability
from .process import TaskFamilyFunctionalAggregate, aggregate_by_task_family
...
def _build_features(
    aggregates: list[TaskFamilyFunctionalAggregate],
) -> dict[str, Any]:
...
def _band(
    ...
    `run_hibayes.py:329`; the functional axis emits posterior.json directly
    with a `str`, so no `.value` extraction is needed here. See Implementation
    Notes.
    """
    if n_total < 3:
        return "TooUncertain"
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--out-dir",
        type=Path,
        default=Path("/work/out/hibayes_functional_usefulness"),
    )
