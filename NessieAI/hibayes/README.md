...
operator code, 34 under `NessieAI/tests/hibayes/`, and 19 vendored under
`NessieAI/hibayes/fit/vendor/`. The package docstring frames the whole thing as one
plan increment (`NessieAI/hibayes/__init__.py:1`).
...
the paid lane. `NessieAI/hibayes/seam_inventory.py:1` walks the package's own AST to
list every provider seam and flag any that is not gated
(`NessieAI/hibayes/seam_inventory.py:159-165`).
...
`NessieAI/hibayes/artifact_validity_set3_final.csv` carries 298 arm rows and
`NessieAI/hibayes/artifact_detail_set3_final.csv` 256 artifact rows, both counted with
`wc -l` on 2026-09-03 and both matching the totals the generating module's docstring
states (`NessieAI/hibayes/artifact_validity_proposal.py:55`).
...
(`dmac/settings.py:497-499`). See `NessieAI/hibayes/CLAUDE.md` for that exact command
...
- 8, spread over `NessieAI/tests/hibayes/test_v4_8_mysql.py` and
...
- 2, in `NessieAI/tests/hibayes/test_v14_quality_hierarchical.py`, want the sampler
...
- `nextseek_api/assistant/models_db.py` for every ORM table written here. Seven modules
  import it at module scope, counted 2026-09-03: `NessieAI/hibayes/export.py:4`,
...
- `nextseek_api/cc_assistant/router.py`, bound as a module-scope constant at
  `NessieAI/hibayes/seam_inventory.py:21` and rebound at
  `NessieAI/hibayes/seam_inventory.py:125`, then parsed as AST. Nothing is imported from
...
- Production code: exactly three modules, all in the Container-CC package —
  ...
  outside `NessieAI/hibayes/` and `scripts/` imports this package.
...
See `NessieAI/hibayes/CLAUDE.md` for the invariants, the traps and the one command.
