...
def runAuthFileSubmission(sampledata, token):
    from api_fileSubmit import runAuthPostFile
    ...
    for field, value in sdata.items():
        if SAMPLE_FILE_ACCESSOR_NAME in field:
            ...
            if linkField in sdata:
                filename = sdata[field]
                flink = sdata[linkField]
                fullfilename = flink
                if len(fullfilename.strip()==0):
                    msg = "Warning: file not available:" + fullfilename
                    print(msg)
                    continue
                ...
                print("Submit data file: " + fullfilename)
                resultdic = runAuthPostFile(url, fullfilename, token)
    
    return resultdic

def runAuthSampleBatchSubmission(sampleDiclist, diclist_ins, token, userInfo, submitDfile=True):
    ...
    for dici in sampleDiclist:
        ...
        for sampleType in sampleTypes:
            ...
            if status==1:
                minfo = uid
                if submitDfile:
                    resultdic2 = runAuthFileSubmission(sampledata, token)
            else:
                minfo = msg
...
def runAuthSampleSubmission(sampledata, sampleType, token, userInfo):
    resultdic = {'msg':'test', 'status':1, 'UID':'test', 'data':''}
    return resultdic
        
def main():
    parser = argparse.ArgumentParser(description="Submit a sample sheet through API call to the Seek system",
    ...
    parser.add_argument('-t', '--type', 
                        help="The sample type for submitting sample meta data, such as . D.SEQ etc.",
                        default=None)
    
    parser.add_argument('sampledata', help="Sample meta data in a json dictionary")
    ...
    runAuthSampleSubmission(sampledata, sampleType, token)
    return
