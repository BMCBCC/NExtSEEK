...
SAMPLETYPE_CONNECTIONS_DESC = (
    ...
    "that makes it. Superuser only.\n\n"
    "**USE WHEN:** The user wants the assay-connection map for a project, investigation, "
    "study or sample type -- 'what connects to what, and by which assay' -- as data, a CSV, "
    "a static diagram, or an interactive network.\n\n"
    "**DO NOT USE WHEN:** The caller is not a superuser. This endpoint is deliberately NOT "
    "project-scoped to the caller: it reports the schema-level shape of the graph across "
    "every project a selector reaches, so a lab user would see that connections exist in "
    "projects they cannot otherwise read. It returns sample TYPE codes and assay titles "
    "only, never sample UIDs or record data. For per-sample lineage inside the caller's own "
    "projects, use `/entity_tree/lineage` instead.\n\n"
    "**ACCEPTS:** Pick a scope. At least one is required, so an empty querystring can never "
    "dump the whole graph by accident.\n\n"
    "| scope | parameters |\n"
    "|---|---|\n"
    "| whole graph | `all_conns=yes` |\n"
    "| investigation | `graph_inv_id` (Investigation.id) / `seek_inv_id` (its PROJECT id, so "
    "it spans that project's investigations) / `investigation_name` |\n"
    "| study | `study_id` / `study_name` |\n"
    "| sample type | `sample_type`, optionally with `direct_connections` |\n\n"
    "`sample_type` COMBINES with an investigation or a study. All filters are AND-ed, so an "
    "investigation plus a study it does not contain returns nothing rather than the union.\n\n"
    "`direct_connections` defaults to **false**, meaning the whole tree rooted at "
    "`sample_type` is walked (NHP -> PAV -> TIS -> DNA and on down). NHP returns 38 "
    "connections that way against 3 direct. Set it true for direct edges only. It has no "
    "effect without a `sample_type`, since there is no root to walk from.\n\n"
    "Two things worth knowing about study selectors. The graph and SEEK agree on study IDs, "
    "so there is one `study_id` rather than a graph/seek pair -- but the graph holds 56 "
    "Study nodes against SEEK's 48, so 8 ids resolve only in the graph. Their TITLES, by "
    "contrast, disagree on 44 of the 48 shared studies, so `study_name` tries the graph's "
    "title first and SEEK's second.\n\n"
    "`output_format` is `json` (default), `csv`, `svg`, or `html`. `layout` (`radial` or "
    "`layered`) overrides the SVG default, which is radial for a scope and layered for a "
    "bare sample type.\n\n"
    "**RETURNS:** `parent_sample_type`, `child_sample_type`, `internal_assay`, `n_edges`, "
    "and the clade of each endpoint. As CSV: `SampleType,isParent,SampleType,InternalAssay,"
    "Edges`. As SVG: a static clade-coloured diagram. As HTML: an interactive "
    "cytoscape/dagre network.\n\n"
    "Assay labels are not exhaustive on edges predating the #118 backfill: an edge kept only "
    "the lowest-id assay its endpoints shared and dropped the rest, so specific "
    "(parent, child, assay) triples can be missing.\n\n"
    "**TRIGGER PHRASES:** sample type connections, assay connections, what connects to what, "
    "connection map, sample type graph for a project or study, clade diagram\n\n"
    "**EXAMPLES:**\n"
    "- 'Show me all the sample type connections in the Impactb investigation'\n"
    "- 'What connects to what in study 14?'\n"
    "- 'Which assays come off NHP, all the way down?'\n"
