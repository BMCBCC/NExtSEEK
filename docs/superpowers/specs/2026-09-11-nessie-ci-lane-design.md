...
| 3 | The questions | Four, in one chat, in this order: "What can you do?", "What mice are treated with NDMA?", "What studies are in IMPACT?" (NS), then "Make me a histogram of NHP species" (CC). All routed, none forced | Covers the system-agent, API and graph paths and the CC engine, and tests the real BAML router. NS first and CC last, so sticky CC cannot capture an NS question |
...
| "Make me a histogram of NHP species" | `container_cc`, `baml` | The reply and at least one artifact link show | `cc_turn_meta.model_id` is not null; no 403 and no `query_error`; the CC artifact download works for one file and for the zip; the transcript comes back as ndjson; the reported cost is recorded |
