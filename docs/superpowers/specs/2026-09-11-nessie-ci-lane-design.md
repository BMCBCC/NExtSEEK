...
Not the goal: answer accuracy. That stays with `NessieAI/tests/nessie_tests/`, which the owner drives by hand. A later
...
| # | Decision | Choice | Why |
|---|---|---|---|
| 1 | What a green run proves | "This build did not break Nessie" | Quality belongs to `NessieAI/tests/nessie_tests/` |
...
| 3 | The questions | Four, in one chat, in this order: "What can you do?", "What mice are treated with NDMA?", "What studies are in IMPACT?" (NS), then "Make me a histogram image of NHP species" (CC). All routed, none forced | Covers the system-agent, API and graph paths and the CC engine, and tests the real BAML router. NS first and CC last, so sticky CC cannot capture an NS question |
...
| "Make me a histogram image of NHP species" | `container_cc`, `baml` | The reply and at least one artifact link show | `cc_turn_meta.model_id` is not null; no 403 and no `query_error`; the CC artifact download works for one file and for the zip; the transcript comes back as ndjson; the reported cost is recorded |

The path for questions 2 and 3 is read from the bundle the turn registered: the graph branch records its
bundle as a graph query (the graph branch of `NessieAI/chat_nextseek/src/chat_nextseek/orchestrator.py`), and the
...
- Stack health gains a Nessie-prerequisites check that runs only when the lane is on: the Bedrock proxy's
  ...
  failure here fails the run before the suite starts and names the missing piece (decision 6). The token
  file is `NessieAI/docker/bedrock-proxy/proxy-secret.env`; the check reads it through the existing path
  constant (`PROXY_SECRET_ENV` in `startup/lib/layout.py`), never by a literal path.
...
### 3.5 Frontend (`NessieAI/chat_frontend/`)
...
- **A spreadsheet is checked only when the turn offers one.** The first paid run (2026-09-11) found that
  ...
  `NessieAI/tests/api/test_excel_export.py`. The lane requires every table or xlsx file a turn
...
Paid quality runs; measuring NS cost; a startup command for the `NessieAI/tests/nessie_tests/` full tier; the write gate
