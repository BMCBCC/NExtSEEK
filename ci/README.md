...
| **xfail / XPASS** | A route broken today carries an `xfail` reason and reports `xfailed`. Because `expect` names the status a *working* route returns, the day the defect is fixed that same entry reports **XPASS**, which is the signal to delete the pin rather than a new failure (`ci/routes.py:44-49`, `ci/smoke/README.md:266-269`) |
